# ПЛАНИРОВАНИЕ ПРОЕКТА

## 🗺️ Дорожная карта разработки

### ЭТАП 1: Подготовка и структура

#### Спринт 1.1: Настройка окружения
- [ ] Создать файловую структуру проекта
- [ ] Подключить Tailwind CSS через CDN
- [ ] Создать базовую HTML разметку
- [ ] Добавить мета-теги и viewport

#### Спринт 1.2: Дизайн-система и цвета
- [ ] Настроить цветовую палитру (CSS переменные)
- [ ] Определить типографику и размеры шрифтов
- [ ] Создать базовые классы для кнопок
- [ ] Настроить стили карточек и контейнеров
- [ ] Определить отступы и размеры элементов

**Критерии завершения этапа:**
- ✅ Файлы index.html и script.js созданы
- ✅ Tailwind CSS подключен и работает
- ✅ HTML валидный и совместимый с Tilda
- ✅ Цветовая схема и дизайн-система настроены

### ЭТАП 2: Основные секции

#### Спринт 2.1: Header и Hero
- [ ] Создать липкую навигацию
- [ ] Добавить логотип (placeholder)
- [ ] Создать меню (Цены, Гарантия, Контакты)
- [ ] Добавить телефон с графиком работы
- [ ] Создать главный заголовок
- [ ] Добавить подзаголовок
- [ ] Создать CTA кнопку с ссылкой #forma
- [ ] Добавить изображение техника (placeholder)

**Критерии завершения этапа:**
- ✅ Header зафиксирован при скролле
- ✅ Все элементы навигации на месте
- ✅ Hero секция привлекает внимание
- ✅ CTA кнопка ведет на #forma

#### Спринт 2.2: Карточки оборудования
- [ ] Создать секцию "Что сломалось?"
- [ ] Добавить 3 карточки: Принтеры, МФУ, Плоттеры
- [ ] Добавить изображения (placeholder)
- [ ] Сделать карточки кликабельными (#forma)

**Критерии завершения этапа:**
- ✅ 3 карточки созданы и адаптивны
- ✅ Все карточки ведут на #forma
- ✅ Hover эффекты работают

#### Спринт 2.3: Бренды и неисправности
- [ ] Создать бегущую строку с брендами
- [ ] Добавить логотипы брендов (placeholder)
- [ ] Создать сетку частых неисправностей
- [ ] Добавить иконки для каждой неисправности
- [ ] Добавить цены из данных
- [ ] Сделать каждую карточку кликабельной (#forma)

**Критерии завершения этапа:**
- ✅ Бегущая строка работает плавно
- ✅ Все неисправности с ценами отображены
- ✅ Каждая карточка ведет на #forma

### ЭТАП 3: Конверсионные блоки

#### Спринт 3.1: Скидка с таймером
- [ ] Создать секцию со скидкой 25%
- [ ] Добавить заголовок с призывом к действию
- [ ] Создать countdown timer
- [ ] Добавить CTA кнопку (#forma)

**Критерии завершения этапа:**
- ✅ Таймер обратного отсчета работает
- ✅ Секция выделяется ярким дизайном
- ✅ CTA кнопка ведет на #forma

#### Спринт 3.2: Система табов с услугами
- [ ] Создать навигацию табов (Принтеры, МФУ, Плоттеры)
- [ ] Добавить контент для каждого таба
- [ ] Интегрировать данные из Услуги.html
- [ ] Сделать каждую услугу кликабельной (#forma)
- [ ] Добавить JavaScript переключения табов

**Критерии завершения этапа:**
- ✅ Табы переключаются корректно
- ✅ Все услуги с ценами отображены
- ✅ Каждая услуга ведет на #forma

### ЭТАП 4: Блоки доверия

#### Спринт 4.1: Гарантия и география
- [ ] Создать секцию гарантии
- [ ] Добавить изображение гарантийного талона (placeholder)
- [ ] Описать преимущества гарантии
- [ ] Создать секцию выезда по Москве
- [ ] Добавить карту с радиусом (placeholder)
- [ ] Описать зоны обслуживания

**Критерии завершения этапа:**
- ✅ Блоки доверия созданы
- ✅ Информация о гарантии понятна
- ✅ География выезда объяснена

#### Спринт 4.2: Договор
- [ ] Создать секцию официального договора
- [ ] Добавить изображение договора (placeholder)
- [ ] Описать процесс заключения договора
- [ ] Подчеркнуть юридическую надежность

**Критерии завершения этапа:**
- ✅ Блок договора добавляет доверие
- ✅ Процесс работы объяснен

### ЭТАП 5: JavaScript функциональность

#### Спринт 5.1: Интерактивность
- [ ] Реализовать sticky header
- [ ] Добавить smooth scroll для навигации
- [ ] Создать систему переключения табов
- [ ] Реализовать countdown timer
- [ ] Добавить hover эффекты

**Критерии завершения этапа:**
- ✅ Все интерактивные элементы работают
- ✅ JavaScript совместим с Tilda
- ✅ Нет конфликтов в консоли

### ЭТАП 6: Финальная проверка

#### Спринт 6.1: Тестирование и оптимизация
- [ ] Проверить все ссылки на #forma
- [ ] Протестировать адаптивность 320px-1920px
- [ ] Проверить совместимость с Tilda
- [ ] Оптимизировать производительность
- [ ] Валидировать HTML и CSS

**Критерии завершения этапа:**
- ✅ Код готов к интеграции в Tilda T123
- ✅ Все функции работают корректно
- ✅ Адаптивность идеальная на всех устройствах

---

## 🎨 ЦВЕТОВАЯ СХЕМА И ДИЗАЙН-СИСТЕМА

### Основная цветовая палитра

**Основные цвета:**
- **Primary Blue:** `#2563eb` - основной синий (доверие, надежность)
- **Primary Blue Dark:** `#1d4ed8` - темный синий (hover состояния)
- **Accent Orange:** `#ea580c` - акцентный оранжевый (CTA кнопки)
- **Accent Orange Dark:** `#dc2626` - темный оранжевый (hover для CTA)

**Нейтральные цвета:**
- **White:** `#ffffff` - белый (фон, карточки)
- **Gray 50:** `#f9fafb` - светло-серый (альтернативный фон)
- **Gray 100:** `#f3f4f6` - серый (границы, разделители)
- **Gray 600:** `#4b5563` - темно-серый (вторичный текст)
- **Gray 900:** `#111827` - черный (основной текст)

**Статусные цвета:**
- **Success Green:** `#059669` - зеленый (гарантии, успех)
- **Warning Yellow:** `#d97706` - желтый (внимание, скидки)

### CSS переменные для Tilda
```css
:root {
  --primary-blue: #2563eb;
  --primary-blue-dark: #1d4ed8;
  --accent-orange: #ea580c;
  --accent-orange-dark: #dc2626;
  --text-primary: #111827;
  --text-secondary: #4b5563;
  --bg-white: #ffffff;
  --bg-gray: #f9fafb;
  --success-green: #059669;
  --warning-yellow: #d97706;
}
```

### Применение цветов

**Кнопки CTA:**
- Фон: `#ea580c` (Accent Orange)
- Hover: `#dc2626` (Accent Orange Dark)
- Текст: `#ffffff` (White)

**Заголовки:**
- H1: `#111827` (Gray 900) 
- H2-H6: `#2563eb` (Primary Blue)

**Основной текст:**
- Основной: `#111827` (Gray 900)
- Вторичный: `#4b5563` (Gray 600)

**Фоны:**
- Основной: `#ffffff` (White)
- Альтернативный: `#f9fafb` (Gray 50)
- Header: `#2563eb` (Primary Blue)

**Карточки:**
- Фон: `#ffffff` (White)
- Границы: `#f3f4f6` (Gray 100)
- Hover: легкая тень + подъем

**Ссылки и акценты:**
- Обычные ссылки: `#2563eb` (Primary Blue)
- Hover ссылки: `#1d4ed8` (Primary Blue Dark)
- Цены: `#059669` (Success Green)
- Скидки: `#d97706` (Warning Yellow)

### Типографика

**Заголовки:**
- H1: `font-size: 3rem (48px)`, `font-weight: 900`, цвет `#111827`
- H2: `font-size: 2rem (32px)`, `font-weight: 700`, цвет `#2563eb`
- H3: `font-size: 1.5rem (24px)`, `font-weight: 600`, цвет `#2563eb`

**Основной текст:**
- Body: `font-size: 1rem (16px)`, `font-weight: 400`, цвет `#111827`
- Small: `font-size: 0.875rem (14px)`, цвет `#4b5563`

**Кнопки:**
- Размер: `font-size: 1rem (16px)`, `font-weight: 600`
- Цвет текста: `#ffffff`

### Компоненты

**CTA кнопки:**
```css
.btn-cta {
  background: #ea580c;
  color: #ffffff;
  padding: 12px 24px;
  border-radius: 8px;
  font-weight: 600;
  transition: all 0.2s;
}
.btn-cta:hover {
  background: #dc2626;
  transform: translateY(-2px);
  box-shadow: 0 10px 25px rgba(234, 88, 12, 0.3);
}
```

**Карточки:**
```css
.card {
  background: #ffffff;
  border: 1px solid #f3f4f6;
  border-radius: 12px;
  padding: 24px;
  transition: all 0.2s;
}
.card:hover {
  transform: translateY(-4px);
  box-shadow: 0 10px 25px rgba(0, 0, 0, 0.1);
}
```

**Цены:**
```css
.price {
  color: #059669;
  font-weight: 600;
  font-size: 1.125rem;
}
```